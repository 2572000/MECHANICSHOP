...
{
    public class IssueInvoiceCommandHandler(IAppDbContext context,
        ...
        TimeProvider dateTime):IRequestHandler<IssueInvoiceCommand, Result<InvoiceDto>>
    {
        ...
        public async Task<Result<InvoiceDto>> Handle(IssueInvoiceCommand request, CancellationToken ct)
        {
            ...
            {
                _logger.LogWarning("Invoice creation failed for WorkOrder {WorkOrderId}. Errors: {@Errors}",
                    workOrder.Id, createInvoiceResult.IsError);
