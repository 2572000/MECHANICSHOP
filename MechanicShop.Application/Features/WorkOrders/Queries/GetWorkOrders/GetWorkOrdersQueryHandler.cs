...
namespace MechanicShop.Application.Features.WorkOrders.Queries.GetWorkOrders
{
        ...
        :IRequestHandler<GetWorkOrdersQuery,Result<PaginatedList<WorkOrderListItemDto>>>
    {
        ...
        public async Task<Result<PaginatedList<WorkOrderListItemDto>>> Handle(GetWorkOrdersQuery request, CancellationToken ct)
        {
            var workOrdersQuery = _context.Workorders.AsQueryable()
                .Include(x => x.Vehicle).ThenInclude(v => v.Customer)
                .Include(x => x.Labor)
                .Include(x => x.RepairTasks).ThenInclude(rt => rt.Parts)
                .Include(x => x.Invoice)
                .AsQueryable();
